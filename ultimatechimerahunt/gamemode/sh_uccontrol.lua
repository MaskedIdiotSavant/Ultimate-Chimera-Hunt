...
function meta:DoubleJump()
	...
	self:SetSprint((self:GetSprint() - (DJump_Penalty * (1 + (self:GetNWInt("DJumpNum", 0) * .66)))));
...
function meta:CanPressButton()
	...
	if (!uc:IsValid() || !uc:Alive() || self:IsScared() || self:IsGhost() || self:IsSalsa() || self:GetNWBool("Swiped", false)) then
		return false;
	end
	...
	if (tr.Entity == uc && ((dis <= 80 && !tblbool) || self:GetGroundEntity() == uc)) then
		return true;
	end
