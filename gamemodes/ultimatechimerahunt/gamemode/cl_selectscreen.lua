...
function PANEL:Init()
	...
	self.btnCancel = vgui.Create( "DButton", self )
		...
		self.btnCancel.DoClick = function() self.ChooseTeamLogo:SetVisible(false) self:Remove() end
	...
	self.ChooseTeamLogo:SetImage("UCH/chooseteam");
...
function PANEL:AddSelectButton( strName, fnFunction, txt, color )
	...
	btn:SetText( strName )
	btn:SetSize( 200, 30 )
	btn.DoClick = function() fnFunction() surface.PlaySound( Sound("buttons/lightswitch2.mp3") ) self:Remove() end
	btn:SetColor( color )
